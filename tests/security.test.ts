...
test("review paths stay inside the project and reject sensitive files", () => {
  ...
  try {
    assert.equal(resolveSafeReviewPath(root, "safe.ts", "file"), join(root, "safe.ts"));
    assert.throws(() => resolveSafeReviewPath(root, ".env", "file"), /sensitive/);
    assert.throws(() => resolveSafeReviewPath(root, "../outside.ts", "file"), /outside/);
    assert.throws(() => resolveSafeReviewPath(root, "escape.ts", "file"), /outside/);
  } finally {
...
test("invalid configuration values cannot escape policy ranges", () => {
  ...
  try {
    ...
    assert.equal(config.dimensionWeights.readability, DEFAULTS.dimensionWeights.readability);
  } finally {
    rmSync(parent, { recursive: true, force: true });
  }
});
